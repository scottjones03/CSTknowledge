...
# //*[@id="radix-:r8:"]/div/div/div[4]/button/div
chatgpt_nextbox = (By.XPATH, '/html/body/div[3]/div/div/div/div/div/div[4]/button[2]')
chatgpt_donebox = (By.XPATH, '//*[@id="radix-:r8:"]/div/div/div[4]/button[2]/div')
chatgpt_gpt4 = (By.ID, 'radix-:r8:')
...
class ChatGPT:
    ...
    def __init_browser(self) -> None:
        ...
        try:
            self.driver = uc.Chrome(options=options, version_main=111, headless=True)
    ...
    def send_message(self, message: str, stream: bool = False) -> dict:
        ...
        if not matches:
            ...
            time.sleep(5)
            matches = pattern.search(self.driver.current_url)
        conversation_id = matches.group() if matches is not None else None
    ...
    def close_chat_research_popup(self) -> None:
        ...
        """) 
        button = WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "button.btn-neutral")))
        button.click()
        time.sleep(random.uniform(1, 3))
        button = WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(self.driver.find_elements(By.CSS_SELECTOR, "button.btn-neutral")[1]))
        button.click()
        # self.driver.find_elements(By.CSS_SELECTOR, "button.btn-neutral")[1].click()
        time.sleep(random.uniform(1, 3))
        button = WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(self.driver.find_elements(By.CSS_SELECTOR, "button.btn-primary")[1]))
        button.click()
        # self.driver.find_elements(By.CSS_SELECTOR, "button.btn-primary")[1].click()
